...
﻿版本：4.1（重大更新，与旧版本的词库格式不兼容！）
1、把字库生成器整合进了软件中，所以数据库的结构有了变化，在更新后首次运行的时候 ，由于要进行数据的升级，比较耗时，耐心等一下，只是第一次运行的时候这样。最好大家先备份原来的数据！然后重新导入系统自带的数据，然后再再导入自定义的词库。至于怎么编写自已的词库，请参看后面的帮助！
2、数据文件的编写及所谓“二级替换”的内容，请参看帮助文件！
...
%w :删除刚刚替换上去的内容
%% :显示%本身
其中最最重要的是%b和%B，%b前面已经说到了妙用在于删除前面的空格，%B的妙用在何处呢？你能猜到吗？旧版本的本软件自带的字库中还有如下的条目，想想都实现什么功能呢？
$,%b%b
$$,%b%b%b
$$$,%b%b%b%b
$$$$,%b%b%b%b%b
...
有了替换，符号输入不是问题，只要设好替换条目就是了。在软件自带的“五笔拼音”编码中，符号的定义是：
$p,%b　　
$s,%b§
z$,%b￥
z$$,%b$
...
z加上两个对应的字母，就是对应的英文符号。比如：
zmm　　.
剩下的几个，只好记一记了！其他更多的符号，请输入zfh试试，有好几类！
大家可能也注意到了，上面的条目中有两个特殊的东西，即“#COMMA#”和“#SHARP#”，它们是“,”和“#”号的转义表示，因为在本软件中它们有特殊的含义，所以在自编词库时如果需要让软件“输出”这两个字符，就需要用到转义。

七、自编词库（从版本4.1起才适用本节的内容）
从4.1版本起，我把词库生成功能整合进了软件中，也就是说上面介绍的原理方面的内容，在实际使用中用不上了（哈哈！！）。现在可能直接导入词库的源文件了！源文件的格式要求如下：
A、源文件的每一行的格式为“编码,候选词1,候选词2……”，不要需要在加上“%b”或者“%B”的宏了，在导入词库的时候软件会自动给加上的。比如：
	gfpk,迂,天壤之别
	geng$,更,耿,耕,庚,羹,梗,哽,赓,鲠,埂,绠,颈
B、如果要指代英文逗号或者英文井号，请用转义名称#COMMA#和#SHARP#，比如：
	znn,#COMMA#
	zqq,#SHARP#
C、源文件最好使用utf-8编码，如果导入后有乱码，那就换一种试试。
D、关于“二级替换”
“二级替换”是从4.1版本起才新加的功能。比如，需要输入全家人的身份证号码，当然我们可以给每个人编一个码：
$baba,110154214525456
$mama,110154214525457
$baobao,110154214525458
在人数少的时候还可以，人一多编码就很难记了。此时就需要用“二级编码”来解决，实现类似于一个二级菜单功能，具体的编码如下：
[twolevel]
$sfz,我,妻,宝,父,母,哥,姐
我,110154214525456
妻,110154214525457
...
姐,110154214525453
[/twolevel]
首先，需要用[twolevel]和[/twolevel]包起来。其次，第一行是“一级菜单”，后面的是“二级菜单”。第三，后面的行对应“一级菜单”中的每一项。第四、如果需要查看“二级编码”，在软件中查询“twolevel”将列出本词库中所有的二级替换，不同的分组由编码前显示的数字来区分。
上面这一段编码实现的功能是：输入“$sfz”然后按空格，“$sfz”被替换成“➊我➋妻➌宝➍父➎母➏哥➐姐”，注意“姐”后面是没有空格的（被%B删掉了），此时按e加空格，不会再现“妻”，而是直接出现“妻”对应的身份证号码“110154214525457”。
