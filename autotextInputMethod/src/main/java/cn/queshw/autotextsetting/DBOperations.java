...
public class DBOperations {
	...
	private final GenAutotext ga = new GenAutotext();
	...
	public int addOrUpdateMethodItem(String name, int isDefault, int id) {
		...
		if (!TextUtils.isEmpty(name)) {// 如果不为空
			...
			if (cursor.getCount() == 0) {// 如果原来没有此条记录，则新增记录
				...
				// 创建与新增输入法的相连的词库表
				String rawTableName = "raw" + id;
				String autotextTableName = "autotext" + id;
				// 1、创建raw表
	...
	public ArrayList<RawItem> searchRawItems(String table, String searchText, int limit, int offset) {
		...
		if (searchText.equals("twolevel"))
			...
			sql = "select * from " + table + " where code like '" + searchText + "%' escape '/' order by code limit " + limit + " offset "
					+ offset;
	...
	public void addOrSaveRawItem(int methodId, String code, String candidate, int id) {
		String rawTableName = "raw" + methodId;
		...
		if (!TextUtils.isEmpty(code) && !TextUtils.isEmpty(candidate)) {// 如果有一个为空，则什么都不干
			...
			String sql = "select id from " + rawTableName + " where id = " + id;
			Cursor cursor = db.rawQuery(sql, null);
			...
			if (cursor.getCount() == 0) {// 说明为新增记录
				...
				sql = "update " + rawTableName + " set code = '" + code + "', candidate='" + candidate + "' where id = " + id;
				db.execSQL(sql);
	...
	private void regenAutotext(int methodId, int id) {
		...
		String rawTableName = "raw" + methodId;
		String autotextTableName = "autotext" + methodId;
		String sql;

		// 先删除现有的对应的项目
		sql = "delete from " + autotextTableName + " where rawid=" + id;
		db.execSQL(sql);
		...
		if (id < 0) {// 需要更新的为二级替换项
			sql = "select * from " + rawTableName + " where twolevel = " + id;
		} else {// 需要更新的不是二级替换项
			sql = "select * from " + rawTableName + " where id = " + id;
		}
		Cursor cursor = db.rawQuery(sql, null);
		while (cursor.moveToNext()) {
			tempInput.clear();
	...
	public void deleteRawItem(int methodId, RawItem item) {
		String rawTable = "raw" + methodId;
		String sql = "delete from " + rawTable + " where id = " + item.getId();
		db.execSQL(sql);
	...
	public int getMaxInputLength(int methodId) {
		String sql = "select max(length(input)) from autotext" + methodId;
		Cursor cursor = db.rawQuery(sql, null);
		cursor.moveToNext();
