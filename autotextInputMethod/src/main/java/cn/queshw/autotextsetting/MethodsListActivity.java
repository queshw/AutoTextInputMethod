...
public class MethodsListActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		methodsListview = findViewById(R.id.methods_listview_activity_main);
		...
		methodsListview.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				RelativeLayout tempslideMenu = view.findViewById(R.id.slidemenu_linearlayout_methoditem);
				isdefaultImageView = view.findViewById(R.id.isdefalut_imageview_methoditem);// 根据ID获得相应的左划菜单上的项目，为以后设置点击监听器作准备
				editImageView = view.findViewById(R.id.edit_imageview_methoditem);
				deleteImageView = view.findViewById(R.id.delete_imageview_methoditem);
	...
	private void addOrEdit(final int id) {
		...
		ed = view.findViewById(R.id.name_add_or_edit_method);
		sw = view.findViewById(R.id.isdefault_add_or_edit_method);

		if (id != ADD) {
			MethodItem item = dboper.getMethodItem(id);
			ed.setText(item.getName());
			isDefault = item.getIsDefault();
			sw.setChecked(item.getIsDefault() == MethodItem.DEFAULT);
		}
